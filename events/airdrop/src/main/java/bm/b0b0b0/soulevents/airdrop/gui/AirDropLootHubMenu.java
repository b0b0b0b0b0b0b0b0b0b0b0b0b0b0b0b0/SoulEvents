...
import bm.b0b0b0.soulevents.airdrop.config.settings.LootHubGuiSettings;
...
public final class AirDropLootHubMenu implements InventoryHolder {
    ...
    private void persist(LootTableSettings loot, AirDropTypeDefinition definition) {
        LootConfigPersistence.saveLootTable(plugin, definition.lootTableId(), loot);
    }

    private Map<String, String> placeholders(LootTableSettings loot, AirDropTypeDefinition definition) {
        int maxOccupied = normalizeChestSize(loot.chestSize);
