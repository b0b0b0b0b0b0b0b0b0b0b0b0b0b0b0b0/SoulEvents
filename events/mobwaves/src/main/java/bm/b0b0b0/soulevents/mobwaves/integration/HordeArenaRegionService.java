...
public final class HordeArenaRegionService implements ArenaRegionService {

    private final Plugin plugin;
    private final Map<UUID, RegionRef> regions = new HashMap<>();

    public HordeArenaRegionService(Plugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void create(
        ...
        String regionId = WorldGuardConstants.HORDE_REGION_PREFIX + sessionId.toString().substring(0, 8);
    ...
    private void applyArenaFlags(ProtectedRegion region, ArenaWorldGuardSettings settings) {
        ...
        forceAllowFlag(region, "mob-damage");
        forceAllowFlag(region, "pvp");
        forceAllowFlag(region, "damage-animals");
        forceAllowFlag(region, "invincibility");
    }

    private static void forceAllowFlag(ProtectedRegion region, String flagName) {
