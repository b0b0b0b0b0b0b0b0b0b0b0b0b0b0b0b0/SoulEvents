...
public final class SpawnWorldResolver {
    ...
    public SpawnWorldResolver(FlatSurfaceFinder flatSurfaceFinder, SchematicService schematics, SoulEventsApi api) {
        this.locationFinder = new RandomLocationFinder(
                flatSurfaceFinder,
                schematics,
                new ActiveSpawnExclusion(api)
        );
    }
    ...
    public Optional<Location> resolve(HordeTypeSettings type, WorldPlacementGate gate) {
        String spawnWorldName = type.worldPlacement.spawnWorld;
        if (spawnWorldName == null || spawnWorldName.isEmpty()) {
    ...
    public String configuredWorldName(HordeTypeSettings type) {
