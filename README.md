...
|-----|-------------|----------|
| **SoulEvents** | — | Ядро: API, сессии, scheduler, защита, схематики, lang, **статистика игроков**, PlaceholderAPI |
...
**Softdepend (по желанию):** FastAsyncWorldEdit / WorldEdit, WorldGuard, Vault, **PlaceholderAPI**, SkinRestorer (volcano), SoulEvents-MobWaves (для `wave-defense` в airdrop).
...
Профиль волн (`MobWaves/profiles/`) задаёт состав волн, HP мобов и **супербосса** каждой волны. Пока волна не пройдена (убит босс, если включён) — `blocksChest` блокирует открытие сундука. Подробнее — раздел [SoulEvents-MobWaves](#soulevents-mobwaves).
...
4. FAWE/WE + (опционально) WorldGuard, Vault, **PlaceholderAPI**
